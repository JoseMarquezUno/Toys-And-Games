...
namespace WebApiTests
{
    public class ProductImagesControllerInMemoryTests : IClassFixture<CustomWebApplicationFactory<Program>>
    {
        ...
        [Trait("IntegrationTestInMemory", "ProductImagesController")]
        [Fact]
        public async void EnsureProductImageAdditionAndDeletion()
        {
            //Arrange
            var productId = 1;
            ...
            productImagesBeforeAdd.EnsureSuccessStatusCode();
            var productImagesContentBeforeAdd = JsonConvert.DeserializeObject<List<ProductImageDTO>>(productImagesBeforeAdd.Content.ReadAsStringAsync().Result);
            var imageAddedInList = productImagesContentAfterAdd.FirstOrDefault(i => i.ImageBase64 == productImage[0].ImageBase64);
            var productImagesContentAfterDeletion = JsonConvert.DeserializeObject<List<ProductImageDTO>>(productImagesAfterDeletion.Content.ReadAsStringAsync().Result);

            Assert.NotNull(imageAddedInList);
