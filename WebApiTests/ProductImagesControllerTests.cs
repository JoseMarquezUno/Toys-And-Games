...
namespace WebApiTests
{
    public class ProductImagesControllerTests
    {
        ...
        public ProductImagesControllerTests(ITestOutputHelper outputHelper)
        {
            _outputHelper = outputHelper;
            _factory = new WebApplicationFactory<Program>();
        }

        [Trait("IntegrationTest","ProductImagesController")]
        [Fact]
        public async void AddProductImages_InsertsNewProductImageInDBAndSavesFileToImageDirectory_UsesGetProductImages()
        {
            //Arrange
            ...
            productImagesResponse.EnsureSuccessStatusCode();
            var productImagesBeforeAdditionResponseContent = productImagesResponseBeforeAddition.Content.ReadAsStringAsync().Result;
            var productImagesResponseContent = productImagesResponse.Content.ReadAsStringAsync().Result;
            var listProductImages = JsonConvert.DeserializeObject<List<ProductImageDTO>>(productImagesResponseContent);
            var listProductImagesBeforeAddition = JsonConvert.DeserializeObject<List<ProductImageDTO>>(productImagesBeforeAdditionResponseContent);

            var productImagesBeforeAdditionCount = listProductImagesBeforeAddition.Count;
            var productImagesAfterAdditionCount = listProductImages.Count;

            Assert.NotEqual(productImagesBeforeAdditionCount, productImagesAfterAdditionCount);
            Assert.NotEmpty(listProductImages);

            _outputHelper.WriteLine($"Number of images for product {productId} before addition: {productImagesBeforeAdditionCount}");
            _outputHelper.WriteLine($"Number of images for product {productId} after addition: {productImagesAfterAdditionCount}");
        }

        [Trait("IntegrationTest", "ProductImagesController")]
        [Fact]
        public async void DeleteProductImage_DeletesImageFromBdAndDirectory_UsesGetProductImages()
        {
            //Arrange
            var client = _factory.CreateDefaultClient();
